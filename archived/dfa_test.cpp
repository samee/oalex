...
void test() {
  ...
  if(sv.s!="foo") BugMe("Parsed '{}' != 'foo'", sv.s);
}
...
struct Hooks : public GssHooks {
  GssHooksRes reduceList(DfaLabel lbl,SharedListVal lv) override {
    if(lbl==DfaLabel{1}) {
      if(lv->size!=2)
        BugMe("Expecting pair, got sequence size {}", lv->size);
...
void test() {
  ...
  const StringVal& sv=dynamic_cast<const StringVal&>(*v);
  if(sv.s!="foobar") BugMe("Parsed '{}' != 'foobar'", sv.s);
}
...
void test() {
  ...
  for(size_t i=0;i<sizeof(invalid_inputs)/sizeof(*invalid_inputs);++i) {
    ...
    if(v!=nullptr) BugMe("Got {} was expecting null", typeid(*v).name());
  }
}
...
class Hooks : public GssHooks {
 public:
  GssHooksRes reduceList(DfaLabel lbl,SharedListVal lv) override {
    ...
    // elt-comma-elt.
    if(lv->size!=3) BugMe("Got list of size {} != 3", lv->size);
  ...
  GssMergeChoice merge(DfaState en,
    ...
    if(lv1->size!=lv2->size)
      BugMe("Can't merge on mismatching sizes: {} != {}",
            lv1->size, lv2->size);
    if(lv1->size==1) {
      ...
      if(en!=DfaState{4})
        BugMe("Merging singleton, but {} != DfaState{4}", en);
    ...
    if(lv1->size!=3)
      BugMe("Expecting elt-comma-elt. Got size {} != 3 at {}",
            lv1->size, en);
...
const string& unwrapToString(const SemVal* v) {
  ...
  if(const auto* lv=dynamic_cast<const ListVal*>(v)) {
    if(lv->size!=1) BugMe("This can't be a string. Size {} != 1", lv->size);
...
void test() {
  ...
  for(size_t i=0;i<n;++i) {
    ...
    if(resg!=outputs[i])
      BugMe("input[{}] parsed into {}", i, resg);
  }
...
void test() {
  ...
  if(typeid(*v)!=typeid(EmptyVal))
    BugMe("Valid empty string returned strange value: "
