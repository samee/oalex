...
static void fillInNames(vector<Rule>& rules) {
  vector<bool> islookahead(rules.size(), false);
  for(auto& rule : rules) if(auto* orrule = get_if<OrRule>(&rule))
    for(auto& comp : orrule->comps) if(comp.lookidx != -1)
      islookahead[comp.lookidx] = true;

  size_t nc = 1;
  for(size_t i=0; i<rules.size(); ++i)
    if(rules[i].needsName(islookahead[i]) && !rules[i].name().has_value())
